...
class Vehicle(CarlaActorBase):
    def __init__(self, world, transform=carla.Transform(),
                 ...
                 vehicle_type="vehicle.bmw.*"): #ali
                # vehicle_type="vehicle.Mustang"): #ali
        # Setup vehicle blueprint
        vehicle_bp = random.choice(world.get_blueprint_library().filter(vehicle_type)) #ali
        #vehicle_bp = world.get_blueprint_library().find(vehicle_type)
        color = vehicle_bp.get_attribute("color").recommended_values[5]
...
class World():
    def __init__(self, client):
        # self.world = client.get_world() #alic
        self.world = client.load_world('Town04') #ali
        self.map = self.world.get_map() #alic
        print(self.map)
        self.actor_list = []
